...
public class ServerThread implements Runnable {
    ...
    private void handleSendInviteRequest(Request request) {
        Invite invite = (Invite) request.getValue();
        String opponentName = invite.getOpponentName();
        int size = invite.getSize();
        Set<ServerThread> namedServerRunnables = server.getNamedServerThreads();
        for (ServerThread serverRunnable : namedServerRunnables) {
            if (serverRunnable.getName().equals(opponentName)) {
                opponent = serverRunnable;
                break;
            }
        }
        status = "(busy)";
        ...
        server.sendUpdateNameListResponseToAllClient();
        opponent.sendResponseToClient(new Response(ResponseKey.RECEIVE_INVITE, new Invite(name, size)));
    }
